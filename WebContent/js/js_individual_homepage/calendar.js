...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      ...
      var offset_t = $(obj).offset().top + (this.$calendar_today.height() - $(obj).height() -150) / 2;
      var offset_l = $(obj).offset().left+50 + $(obj).width()-190;
      ...
      this.$calendar_today
            .css({left: offset_l +80, top: offset_t})
            .stop()
    ...
    renderDOM: function () { // 渲染DOM
      ...
      for (var i = 0; i < 6; i++) {
        _dateStr += '<li onclick="putdate()" class="item">26</li>'+
                    '<li onclick="putdate()" class="item">26</li>'+
                    '<li onclick="putdate()" class="item">26</li>'+
                    '<li onclick="putdate()" class="item">26</li>'+
                    '<li onclick="putdate()" class="item">26</li>'+
                    '<li onclick="putdate()" class="item">26</li>'+
...
//把日历中的日期填入输入框
	function putdate()
	{
		var t=$(".calendar-today p:eq(0)").text();
	//	alert(t);
		$("#search_input").val(t);
